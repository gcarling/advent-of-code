...
const evenlyDivide = (n1, n2) => {
  const ret = (n1 % n2 === 0) || (n2 % n1 === 0);
  if (ret) {
...
function getRowPairDiff(row) {
  let rowDiff;
  _.each(row, n1 => {
    _.each(row, n2 => {
      if (n1 === n2) return;

      if (evenlyDivide(n1, n2)) rowDiff = _.max([n1 / n2, n2 / n1])
    });
  });
  return rowDiff;
}

const ans2 = _.sum(_.map(input, getRowPairDiff));
