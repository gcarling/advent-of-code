...
fs.readFile('in.txt', 'utf8', function(err, contents) {
  ...
  _.each(_.keys(guardMap), guardID => {
    const guardTotal = _.sum(_.values(guardMap[guardID]));

    if (guardTotal > bestScore) {
      bestScore = guardTotal;
      bestGuard = guardID;
    }
  });
  ...
  _.each(_.keys(guardMap[bestGuard]), time => {
    const val = guardMap[bestGuard][time];
