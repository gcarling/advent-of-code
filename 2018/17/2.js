...
function printGridWindow(grid, offset = 200) {
...
function handleWaterFall(grid, waterStartX, waterStartY) {
  // if we're trying to start somewhere that isn't flowing we will accomplish nothing
  if (!isFlowing(grid[waterStartY][waterStartX])) return;
  ...
  function flowToHole(x, y) {
    ...
    if (!isSolid(grid[y+1][x])) {
      ...
      // if after that waterfall it's solid now, we're good
      if (isSolid(grid[y+1][x])) {
        return false;
      } else {
        return true;
